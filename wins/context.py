...
class Context:
    ...
    def context_define(self, pc, itm, element, trade=False):
        if itm.props['item_type'] in ('wpn_melee', 'wpn_ranged'):
            ...
            self.wins_dict['context'].update_elements_lockpick(pc, itm, element, self.mouse_pointer.xy, trade=trade)
    ...
    def update_elements_skill(self, pc, item, element, mouse_xy, trade=False, log=True):
        ...
        # calculating and rendering text
        bl_text = {
            'desc': item.props['desc'] % getattr(skillfuncs, item.props['function_name'])(self.wins_dict, None, pc, item, (element.tags[0], element.id) ,just_values=True)
        }
    ...
    def update_elements_lockpick(self, pc, item, element, mouse_xy, trade=False, log=True):
        self.win_ui.decoratives.clear()
        self.win_ui.interactives.clear()
